...
# 105 "../src/time_date.c"
 ...
 OSCCONbits.SOSCEN = 0;
 do{int intStat, dmaSusp; intStat=INTDisableInterrupts(); dmaSusp=DmaSuspend(); SYSKEY = 0, SYSKEY = 0xAA996655, SYSKEY = 0x556699AA; (OSCCONSET = 0x00000002); SYSKEY = 0x33333333; DmaResume(dmaSusp); INTRestoreInterrupts(intStat);}while(0);
 OSCCONbits.SOSCEN = 1;
...
int CheckRtccRunning(int secWait)
{
 ...
 for(secCnt=0, fail=0; secCnt<secWait; secCnt++)
 {
  ...
  {
   t1.l=RtccGetTime();
  }while((t1.sec==t0.sec) && (ReadCoreTimer()-tStart)<((40000000L)/2000)*1100);
